...
function satisfiesHardConstraints(
  ...
  if (policy.extraCost > preferences.maxExtraCost) {
    return { ok: false, reason: "허용한 추가 비용을 초과합니다." };
  }
...
export function evaluateAndSelectPolicy(
  ...
  const sorted = [...candidates].sort((a, b) => {
    ...
    if (a.policy.extraCost !== b.policy.extraCost) {
      return a.policy.extraCost - b.policy.extraCost;
    }
