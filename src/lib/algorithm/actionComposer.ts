...
export function composeRecommendation(
  ...
  const action: Action = shouldSwitch
    ? { type: "SWITCH", description: `${target.policy.label} 경로로 전환하세요.` }
    : target.policy.usesTaxi &&
        (target.riskLevel === "DANGER" || target.riskLevel === "LATE")
      ? { type: "PREPARE_TAXI", description: "택시 호출을 준비하세요." }
      : { type: "KEEP", description: `${target.policy.label} 경로를 계속 이용하세요.` };
